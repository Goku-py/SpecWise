...
const rows: RowDef[] = [
  ...
  { label: "RAM", getValue: l => `${l.ramAmount} GB${l.ramUpgradeable ? " (upgradeable)" : " (soldered)"}` },
  { label: "Storage", getValue: l => `${l.storageAmount} GB ${l.storageType}${l.storageExpandable ? " + expandable" : ""}` },
  { label: "Display", getValue: l => `${l.displaySize}"${l.displayResolution ? ` ${l.displayResolution}` : ""}${l.displayRefreshRate > 60 ? ` ${l.displayRefreshRate}Hz` : ""}` },
...
function CompareContent() {
  ...
  if (missingIds.length > 0) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center">
        <h2 className="text-2xl font-semibold">Some laptops are missing</h2>
        <p className="mt-2 text-sm text-muted">
          We couldn&apos;t find these IDs in your latest results:
        </p>
        <p className="mt-2 break-all rounded-lg border border-border bg-card/50 p-3 text-xs text-muted">
          {missingIds.join(", ")}
        </p>
        <Link href="/results">
  ...
  if (laptops.length === 0) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center">
        <h2 className="text-2xl font-semibold">No laptops to compare</h2>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:py-12">
      ...
      <h1 className="mb-1 text-3xl font-bold">Compare Laptops</h1>
      <p className="mb-8 text-sm text-muted">Side-by-side comparison of your selected laptops.</p>

      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px] border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 min-w-[140px] bg-background pr-4 text-left text-sm font-semibold text-muted">Spec</th>
              {laptops.map(l => (
                <th key={l.id} className="min-w-[200px] px-3 pb-4 text-left">
                  <div className="text-xs text-muted">{l.brand}</div>
                  <div className="text-sm font-semibold text-foreground">{l.model}</div>
                  <div className="mt-1"><MatchBadge score={l.matchScore} /></div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map(row => (
              <tr key={row.label} className="border-b border-border">
                <td className="sticky left-0 z-10 bg-background py-3 pr-4 text-sm font-medium text-muted">{row.label}</td>
                {laptops.map((l, idx) => {
                  const val = row.getValue(l)
                  return (
                    <td key={idx} className="px-3 py-3 text-sm text-foreground">
                      {typeof val === "boolean" ? (
                        val ? <Check className="h-4 w-4 text-emerald-500" /> : <XIcon className="h-4 w-4 text-muted" />
                      ) : val}
                    </td>
                  )
                })}
            ...
            <tr>
              <td className="sticky left-0 z-10 bg-background py-4 pr-4" />
              {laptops.map(l => (
